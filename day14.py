...
def get_single_dimension_modulo(
    start: int, velocity: int, dimension: int, steps: int
) -> int:
    return (start + steps * velocity) % dimension


def get_position_after_n_steps(
    init_pos_and_vel: Tuple[Tuple[int, int], Tuple[int, int]],
    dimensions: Tuple[int, int],
    num_steps: int,
) -> Tuple[int, int]:
    start, velocity = init_pos_and_vel
    return (
        get_single_dimension_modulo(start[0], velocity[0], dimensions[0], num_steps),
        get_single_dimension_modulo(start[1], velocity[1], dimensions[1], num_steps),
    )


def get_all_final_positions(
    all_guards: List[Tuple[Tuple[int, int], Tuple[int, int]]],
    dimensions: Tuple[int, int],
    num_steps: int,
) -> List[Tuple[int, int]]:
    results = []
...
def get_final_score(
    final_positions: List[Tuple[int, int]], dimensions: Tuple[int, int]
) -> int:
...
def get_variance_of_coordinates(
...
def get_cluster_of_coordinates(
